...
def label_matcher(y_cluster, labels, return_mapper=False):
    ...
    for x in y_cluster_unique:
        unique, counts = np.unique(labels[y_cluster == x],
                                   return_counts=True)  # get frequency of each gt label in cluster x
...
def latex_report(y_true, y_pred, target_names, include_head=True):
    ...
    for key in target_names:
        line = '\t{}\t& {:.3f} \t& {:.3f} \t\\\\\n'.format(key, np.round(data[key]['precision'], decimals=3),
                                                           np.round(data[key]['recall'], decimals=3))
